...
const Cozinheiro = () => {
  ...
  const [filteredCozinheiros, setFilteredCozinheiros] = useState([]);

  const [showPopup, setShowPopup] = useState(false);
  const [showConfirmationPopup, setShowConfirmationPopup] = useState(false);
  const [selectedCozinheiro, setSelectedCozinheiro] = useState(null);
  ...
  const fetchCozinheiros = async () => {
    try {
      const response = await api.get("/cozinheiro");
      setCozinheiros(response.data);
      setFilteredCozinheiros(response.data);
    } catch (error) {
  ...
  useEffect(() => {
    if (selectedCozinheiro) {
      api
        .get(`/prestadores/${selectedCozinheiro.id}/schedule`)
        .then((response) => {
  ...
  const handleFiltrar = async () => {
    try {
      ...
      if (dataInicial && dataFinal) {
        response = await api.get("/prestadores-disponiveis/cozinheiro", {
          params: {
        ...
        response = await api.get("/cozinheiro");
      }
      ...
      const filtered = data.filter((cozinheiro) => {
        ...
        if (filtroNota)
          matches =
            matches && parseInt(cozinheiro.nota) === parseInt(filtroNota);
        if (precoDe)
      ...
      setFilteredCozinheiros(filtered);
    } catch (error) {
      console.error("Erro ao buscar cozinheiros:", error);
    }
  };
  ...
  const handleCheckAvailability = (cozinheiro) => {
    setSelectedCozinheiro(cozinheiro);
    setShowPopup(true);
  };

  const closePopup = () => {
    setShowPopup(false);
    setSelectedCozinheiro(null);
  ...
  const handleConfirmation = async () => {
    try {
      ...
      const response = await api.post(
        "/contrato",
        {
          prestadorId: selectedCozinheiro.id,
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.content}>
        <Text style={styles.title}>Serviços de Cozinheiro</Text>
        <Text style={styles.subtitle}>
          Contrate cozinheiros experientes para suas necessidades culinárias,
        ...
        <Text style={styles.sectionTitle}>Nossos Cozinheiros</Text>
        {filteredCozinheiros.length > 0 ? (
          filteredCozinheiros.map((cozinheiro) => (
            <View key={cozinheiro.id} style={styles.card}>
              <View style={styles.cardHeader}>
                <Text style={styles.cardTitle}>{cozinheiro.nome}</Text>
              </View>
              <Text style={styles.cardText}>{cozinheiro.titulo}</Text>
              <Text style={styles.cardText}>{cozinheiro.descricao}</Text>
              ...
              <TouchableOpacity
                onPress={() =>
                  Linking.openURL(
                    `https://wa.me/55${cozinheiro.telefone.replace(
                      /[^\d]/g,
                      ""
                    )}`
                  )
              ...
              <TouchableOpacity
                ...
                onPress={() => handleCheckAvailability(cozinheiro)}
              >
          ...
          <Text style={styles.noDataText}>
            Nenhum cozinheiro disponível com os filtros selecionados.
          </Text>
        )}
      </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>
              Disponibilidade de {selectedCozinheiro?.nome}
            </Text>
      ...
      <Modal visible={showConfirmationPopup} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text>
              A contratação do cozinheiro {selectedCozinheiro?.nome} foi
