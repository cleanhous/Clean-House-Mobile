...
export default function Cozinheiro() {
  ...
  async function buscarPrestadoresComFiltros() {
    try {
      ...
      Alert.alert(
        "Erro na Busca",
        "Não foi possível buscar cozinheiros. Verifique sua conexão ou tente mais tarde."
      );
      definirPrestadores([]);
  ...
  function formatarData(diaISO, hora) {
    ...
    const dataBase = typeof diaISO === 'object' && diaISO.dateString ? diaISO.dateString : diaISO;
    if (typeof dataBase !== 'string' || !dataBase.includes('-')) return null;
  ...
  async function confirmarContratacao() {
    if (!dataInicio || !horarioInicial || !dataFim || !horarioFinal) {
      Alert.alert("Dados incompletos", "Selecione período (data de início e fim) e os respectivos horários.");
      return;
    }
    ...
    if (new Date(fimSQL) <= new Date(inicioSQL)) {
        Alert.alert("Período Inválido", "A data/hora final deve ser posterior à data/hora inicial.");
        return;
    }
    ...
    try {
      ...
      Alert.alert(
        "Erro na Contratação",
        erro.response?.data?.message || "Falha ao confirmar. Tente novamente."
      );
      console.error(
  ...
  return (
    ...
    >
      <NavBarHome title={"Cozinheiro"} />
      <View style={estilos.conteudo}>
        <Text style={estilos.titulo}>Serviços de Cozinheiro</Text>
        <Text style={estilos.subtitulo}>
          Encontre os melhores cozinheiros para suas necessidades.
        </Text>
        ...
        <Text style={estilos.subtitulo2}>Nossos Cozinheiros</Text>
        ...
        {prestadoresFiltrados.length > 0 ? (
          ...
          <Text style={estilos.textoNenhum}>
            Nenhum cozinheiro encontrado com os filtros aplicados.
          </Text>
        )}
      </View>
